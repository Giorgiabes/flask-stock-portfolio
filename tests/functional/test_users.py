...
def test_valid_registration(test_client):
    ...
    with mail.record_messages() as outbox:
        ...
        assert response.status_code == 200
        assert (
            b"Thanks for registering, abesadze.george@gmail.com! Please check your email to confirm your email address."
            in response.data
        )
        assert b"Flask Stock Portfolio App" in response.data
        assert len(outbox) == 1
        ...
        assert outbox[0].sender == "flasktestdrivendevelopment@gmail.com"
        assert outbox[0].recipients[0] == "abesadze.george@gmail.com"
...
def test_confirm_email_valid(test_client):
    ...
    token = confirm_serializer.dumps(
        "abesadze.george@gmail.com", salt="email-confirmation-salt"
    )
...
def test_invalid_registration(test_client):
    ...
    response = test_client.post(
        "/users/register",
        data={
            "email": "abesadze.george2@gmail.com",
            "password": "",
        },  # Empty field is not allowed!
        follow_redirects=True,
    )
    assert response.status_code == 200
    assert b"Thanks for registering, abesadze.george2@gmail.com!" not in response.data
    assert b"Flask Stock Portfolio App" in response.data
...
def test_duplicate_registration(test_client):
    ...
    response = test_client.post(
        "/users/register",
        data={
            "email": "abesadze.george@gmail.com",  # Duplicate email address
            "password": "FlaskIsStillGreat!",
        },
        follow_redirects=True,
    )
    assert response.status_code == 200
    assert b"Thanks for registering, abesadze.george@gmail.com!" not in response.data
    assert b"Flask Stock Portfolio App" in response.data
    assert b"ERROR! Email (abesadze.george@gmail.com) already exists." in response.data
...
def test_valid_login_and_logout(test_client, register_default_user):
    ...
    assert response.status_code == 200
    assert b"Thanks for logging in, abesadze.george@gmail.com!" in response.data
    assert b"Flask Stock Portfolio App" in response.data
...
def test_invalid_logout_not_logged_in(test_client):
    ...
    """
    test_client.get(
        "/users/logout", follow_redirects=True
...
def test_user_profile_logged_in(
    test_client, log_in_default_user, unconfirm_email_default_user
):
    """
    GIVEN a Flask application configured for testing and the default user logged in
    ...
    assert b"Email: abesadze.george@gmail.com" in response.data
...
def test_user_profile_not_logged_in(test_client):
    ...
    assert b"Email: abesadze.george@gmail.com" not in response.data
    assert b"Please log in to access this page." in response.data
...
def test_post_password_reset_via_email_page_valid(
    ...
    with mail.record_messages() as outbox:
        response = test_client.post(
            ...
            data={"email": "abesadze.george@gmail.com"},
            follow_redirects=True,
        )
        assert response.status_code == 200
        ...
        assert len(outbox) == 1
        assert (
            outbox[0].subject == "Flask Stock Portfolio App - Password Reset Requested"
        )
        assert outbox[0].sender == "flasktestdrivendevelopment@gmail.com"
        assert outbox[0].recipients[0] == "abesadze.george@gmail.com"
        assert "Questions? Comments?" in outbox[0].html
        assert "flasktestdrivendevelopment@gmail.com" in outbox[0].html
...
def test_post_password_reset_via_email_page_invalid(test_client):
    ...
    with mail.record_messages() as outbox:
        response = test_client.post(
            "/users/password_reset_via_email",
            data={"email": "notabesadze.george@gmail.com"},
            follow_redirects=True,
        )
        assert response.status_code == 200
        assert len(outbox) == 0
...
def test_post_password_reset_via_email_page_not_confirmed(
    ...
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/password_reset_via_email' page is posted to (POST) with an imail address that has not been confirmed
    THEN check that an error message is flashed
    """
    with mail.record_messages() as outbox:
        response = test_client.post(
...
def test_get_password_reset_valid_token(test_client):
    ...
    token = password_reset_serializer.dumps(
        "abesadze.george@gmail.com", salt="password-reset-salt"
    )

    response = test_client.get(
        "/users/password_reset_via_token/" + token, follow_redirects=True
    )
    assert response.status_code == 200
...
def test_get_change_password_not_logged_in(test_client):
    ...
    WHEN the '/users/change_password' page is retrived (GET)
    THEN check that error message is returned to the user
    """
    response = test_client.get("/users/change_password", follow_redirects=True)
    assert response.status_code == 200
    assert b"Please log in to access this page." in response.data
...
def test_post_change_password_logged_in_valid_current_password(
    ...
    user = User.query.filter_by(email="abesadze.george@gmail.com").first()
    assert not user.is_password_correct("testpass123")
    assert user.is_password_correct("testpass321")
...
def test_post_change_password_not_logged_in(test_client):
    """
    GIVEN a Flask application configured for testing and user not logged in
    WHEN the '/users/change_password' page is posted to (POST)
    THEN check an error message is required to the user
    """
    response = test_client.post(
        "/users/change_password",
        data={
